       ...
       echo '<tr>
	        ...
	        $baseir2 = "SELECT * FROM fas_games WHERE category='$catid1' and active='1' ORDER BY rand() LIMIT 0,".$limitboxgames ;
	      	...
	      	echo '<a href=\''.$playlink.'\' title=\''.$row['name'].'\'>';
				      		...
				      		if($row['type'] == 1){
				      		echo '	<img src=\''.$domain.'/'.$thumbsfolder.'/'.$row['thumb'].'\' title=\''.$row['name'].'\' alt=\''.$row['name'].'\' width=\'80\' height=\'80\' style=\'margin:3px\' border=\'0\'>';
				      		}else{
				      		echo '	<img src=\''.$row['thumburl'].'\' title=\''.$row['name'].'\' alt=\''.$row['name'].'\' width=\'80\' height=\'80\' style=\'margin:3px\' border=\'0\'>';
				      		}
