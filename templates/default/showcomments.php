...
$limits = ($show - 1) * $max; 
$r = "SELECT * FROM dd_comments WHERE gameid='$ID' and approved ='1' order by date desc LIMIT ".$limits.", ".$max ;

$sqltitle = "gamecomments".$show."-".$ID ;
$totalres = mysql_result($db->query('SELECT COUNT(ID) AS total FROM dd_comments WHERE gameid=\''.$ID.'\' and approved=\'1\' '),0);	
...
$commenter = abs((int) ($in['commenter']));
 ...
 $nv1 = "SELECT * FROM dd_users WHERE userid='$commenter' limit 1 "  ;
