...
<table id="table">
	...
	<tbody>
		<tr>
			...
			<td><a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=games&CID=all\'><img src="panels/admin/img/edit.png" width="24" height="24" alt="edit" title="Edit" /></a></td>
		</tr>';
while($row = mysql_fetch_array($rr)){
	echo '<tr>
			<td width="95%">'.$row['name'].'</td>
			<td><a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=games&CID='.$row['ID'].'\'><img src="panels/admin/img/edit.png" width="24" height="24" alt="edit" title="Edit" /></a></td>
		</tr>';
}
echo'</tbody>
</table>
...
if($row['type'] == 1){
	...
	$dlurl1='<a href=\''.$domain.'/'.$gamesfolder.'/'.$row['file'].'\'><img src="panels/admin/img/download.png" width="24" height="24" alt="download" title="Download" border="0" /></a>';
}else{
	$type = 'Enabled Code';
...
$descriptions = $row['file'];
	...
	echo '<tr>
			...
			<td align=\'center\'>
				<a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=edit&ID='.$row['ID'].'&type='.$row['type'].'\'><img src="panels/admin/img/edit.png" width="24" height="24" alt="edit" title="Edit" border="0" /></a>
				<a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=delete&ID='.$row['ID'].'\'  onclick="return confirm(\'Are you sure you want to delete the game '.$row['name'].'?\')"><img src="panels/admin/img/delete.png" width="24" height="24" alt="delete" title="Delete" border="0" /></a>
				<a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=approve&ID='.$row['ID'].'\'  ><img src="panels/admin/img/approve.png" width="24" height="24" alt="activate" title="Activate" border="0" /></a>
				<a href=\''.$domain.'/index.php?action='.$action.'&case=testgame&gameid='.$row['file'].'\' target=\'_blank\'><img src="panels/admin/img/test.png" width="24" height="24" alt="test" title="Test" border="0" /></a>
            '.$dlurl1.'
			</td>
		</tr>';
};
...
function games(){
global $domain, $db, $gamesfolder, $thumbsfolder, $directorypath, $action;
...
echo '<tr>
		...
		<td><a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=edit&ID='.$row['ID'].'&type='.$row['type'].'\'><img src="panels/admin/img/edit.png" width="24" height="24" alt="edit" title="Edit" border="0" /></a>
			<a href=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=delete&ID='.$row['ID'].'\'  onclick="return confirm(\'Are you sure you want to delete the game '.$row['name'].'?\')"><img src="panels/admin/img/delete.png" width="24" height="24" alt="delete" title="Delete" border="0" /></a>
		</td>
	</tr>';
}
...
$previous = ''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=games&CID='.$CID.'&show='.$pre.'';
$next = ''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=games&CID='.$CID.'&show='.$ne.'';
	...
	for($i = 1; $i <= $totalpages; $i++){
		if($show - $i < '4' || $totalpages - $i < '7'){
			if($i - $show < '4' || $i < '8'){
				...
				$urk = ''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=games&CID='.$CID.'&show='.$i.'';
...
function edit(){
global $domain, $db, $gamesfolder, $thumbsfolder, $action;
$ID = abs((int) $_GET['ID']);
...
<br clear="all">
<form action=\''.$domain.'/index.php?action='.$action.'&case=managegames&cmd=edit&ID='.$ID.'&type=1\' method=\'post\'>
	<table id="table">
		<thead>
			<tr>
