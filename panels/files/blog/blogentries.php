...
$totalpages = ceil($totalres / $max);
	...
	echo '<div class="heading">
        ...
        <br clear="all">
	<a href=\''.$domain.'/index.php?action='.$action.'&case=blogentries&cmd=newentry\' class="button">New Entry</a>
	<table id="table">
		<thead>
			<tr>
				<th>ID</th>
	...
	while($row = $db->fetch_row($r)){
      ...
      $category = $row['category'];
			echo '<tr>
				...
				<td><a href=\''.$domain.'/index.php?action='.$action.'&case=blogentries&cmd=editentry&entryid='.$entryid.'\' ><img src="panels/admin/img/edit.png" width="24" height="24 alt="edit" title="Edit" /></a>
					<a href=\''.$domain.'/index.php?action='.$action.'&case=blogentries&cmd=deleteentry&entryid='.$entryid.'\' onclick="return confirm(\'Are you sure you want to delete this blog '.$row['title'].'?\')"><img src="panels/admin/img/delete.png" width="24" height="24" alt="delete" title="Delete" /></a></td>
			</tr>'; }
echo '</tbody>
</table>
<div class="page-box">'
...
$previous = ''.$domain.'/index.php?action='.$action.'&case=blogentries&page='.$pre.'';
$next = ''.$domain.'/index.php?action='.$action.'&case=blogentries&page='.$ne.'';
	...
	for($i = 1; $i <= $totalpages; $i++){
		if($show - $i < '4' || $totalpages - $i < '7'){
			if($i - $show < '4' || $i < '8'){
				...
				$urk = ''.$domain.'/index.php?action='.$action.'&case=blogentries&page='.$i.'';
...
echo '<form action=\''.$domain.'/index.php?action='.$action.'&case=blogentries&cmd=saveedits\' method=\'post\' >
	<table id="table">
		<thead>
			<tr>
...
<br clear="all">
<form action=\''.$domain.'/index.php?action='.$action.'&case=blogentries&cmd=savenew\' method=\'post\'>
	<table id="table">
		<thead>
			<tr>
				<th colspan="2">Entry Details</th>
...
function savenew(){
	global $domain, $db, $susername, $usrdata, $action;
