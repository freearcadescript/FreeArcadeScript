...
$totalpages = ceil($totalres / $max);
	...
	$r = mysql_query("SELECT * FROM fas_blogentries where author='$useridm' ORDER BY entryid DESC limit $limits,$max");
...
for($i = 1; $i <= $totalpages; $i++){ 
	if($show == $i){
		...
		echo '<a href="'.$domain.'/index.php?action=blogadmin&case=blogentries&page='.$i.'" class="page">'.$i.'</a>&nbsp;';
	}
}
echo '</div>';
...
echo '<form action=\''.$domain.'/index.php?action=blogadmin&case=blogentries&cmd=saveedits\' method=\'POST\' >
	<table id="table">
		<thead>
			<tr>
...
$db->query(sprintf('DELETE FROM fas_blogentries WHERE entryid=\'%u\' and author=\'%s\'', $entryid, $useridm));
